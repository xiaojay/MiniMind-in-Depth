...
对于高维向量，RoPE旋转矩阵是一个块对角矩阵，每2×2块负责一对维度的旋转：

$$\boldsymbol{R}_{\Theta,m}^{d_{model}} = \text{diag}\left(R(m\theta_0), R(m\theta_1), \ldots, R(m\theta_{d/2-1})\right)$$

其中每个$R(m\theta_i)$都是2×2旋转矩阵。
...
实际实现中，为了避免稀疏矩阵运算，RoPE采用元素级操作：

$$\boldsymbol{R}_{\Theta,m}^{d_{model}} \boldsymbol{x} = \boldsymbol{x} \odot \cos(\boldsymbol{m\Theta}) + \text{rotate\_half}(\boldsymbol{x}) \odot \sin(\boldsymbol{m\Theta})$$

其中$\text{rotate\_half}(\boldsymbol{x})$将向量的前半部分和后半部分交换并取负号。
...
$$\theta_0 = 2 \cdot 1.0 = 2.0, \quad \theta_1 = 2 \cdot 0.01 = 0.02$$
